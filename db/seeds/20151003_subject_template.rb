template_type_freelance_blue = SubjectTemplateType.find_by(name: '個人事業主(青色申告)')
account_type_multi = AccountType.find_by(code: 'MULTI')
type_debit = SubjectType.find_by(account_type: account_type_multi, name: '資産の部')
type_credit = SubjectType.find_by(account_type: account_type_multi, name: '負債・資本の部')
type_profit = SubjectType.find_by(account_type: account_type_multi, name: '収入の部')
type_loss = SubjectType.find_by(account_type: account_type_multi, name: '支出の部')

# 個人事業主(青色申告)
FREELANCE_BLUE_SUBJECTS = [
  # 資産の部
...
FREELANCE_BLUE_SUBJECTS.each do |subject|
