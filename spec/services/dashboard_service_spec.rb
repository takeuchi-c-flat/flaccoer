...
RSpec.describe DashboardService do
  ...
  example '#create_dashboard_data_debit_and_credit' do
    ...
    create_subject(type_property, '103', '当座預金').tap { |s| s.balance = create(:balance, top_balance: 10000) }
    ...
    expect(actual[0]).to have_attributes(
      id: property1.id, code: '101', name: '現金',
      top_balance: 50000, debit_total: 100000, credit_total: 5000, last_balance: 145000)
    expect(actual[1]).to have_attributes(
      id: property2.id, code: '102', name: '普通預金',
      top_balance: 500000, debit_total: 0, credit_total: 100000, last_balance: 400000)
    expect(actual[2]).to have_attributes(
      id: 0, code: '103', name: '当座預金',
      top_balance: 10000, debit_total: 0, credit_total: 0, last_balance: 10000)
    expect(actual[3]).to have_attributes(
      id: debt1.id, code: '201', name: '未払金',
  ...
  example '#create_dashboard_data_profit_and_loss' do
    ...
    profit1 = create_subject(type_profit, '301', '売上').tap { |s| s.badget = create(:badget, total_badget: 1000000) }
    ...
    create_subject(type_loss, '403', '租税公課').tap { |s| s.badget = create(:badget, total_badget: 50000) }
    ...
    expect(actual.length).to eq(4)
    expect(actual[0]).to have_attributes(
      id: profit1.id, code: '301', name: '売上',
      total_badget: 1000000, total_result: 150000, achievement_ratio: '15.0%')
    expect(actual[1]).to have_attributes(
      id: loss1.id, code: '401', name: '消耗品費',
      total_badget: 0, total_result: 20000, achievement_ratio: '---%')
    expect(actual[2]).to have_attributes(
      id: loss2.id, code: '402', name: '通信費',
      total_badget: 0, total_result: 45000, achievement_ratio: '---%')
    expect(actual[3]).to have_attributes(
      id: 0, code: '403', name: '租税公課',
      total_badget: 50000, total_result: 0, achievement_ratio: '0.0%')
  end
end
